...
begin
  ...
  return jsonb_build_object(
    ...
    -- Lido dos blocos realmente gravados: recalcular o plano aqui devolveria
    -- vazio, porque os profissionais já estão ocupados com este pedido.
    'professional', (select string_agg(distinct professional.name, ' + ')
      from public.appointments booked
      join public.professionals professional on professional.id = booked.professional_id
      where booked.booking_group_id = group_uuid),
    'assignments', (select coalesce(jsonb_agg(jsonb_build_object(
        'service', service.name, 'professional', professional.name
      ) order by booked.starts_at), '[]'::jsonb)
      from public.appointments booked
      join public.services service on service.id = booked.service_id
      join public.professionals professional on professional.id = booked.professional_id
      where booked.booking_group_id = group_uuid)
  );
